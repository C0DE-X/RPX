...
static constexpr bool isLittleEndian() {
  const long number(1), little_endian(0x00000001);
  return number == little_endian;
}
...
struct Utils {
  ...
  template <typename T>
  static auto fromArithmetic(T const& value, char *buffer) -> std::enable_if_t<isLittleEndian() && std::is_arithmetic_v<T>, void> {
  ...
  static auto fromArithmetic(T const& value, char *buffer) -> std::enable_if_t<!isLittleEndian() && std::is_arithmetic_v<T>, void> {
    std::array<char, sizeof(T)> tmp;
    memcpy(tmp.data(), &value, sizeof(T));
    std::reverse(tmp.begin(), tmp.end());
    memcpy(buffer, tmp.data(), sizeof(T));
  }
